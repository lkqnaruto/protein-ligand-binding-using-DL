...
def one_hot_3D_matrix(one_hot_matrix, atom_type_vector):
    ...
    assert atom_type_matrix.shape == (22, 55, 55)
...
def data_preprocessing_52channel(starting_point, selected_ligand_atom_pair, key_ligand):
    ...
    random_true_ligand_atoms = np.vstack([key_ligand, random_atoms_data])
    ...
    for key_path in range(1):
        ...
        generated_loc = starting_point[1:]
    ...
    return temp, generated_loc, random_atoms_data
...
def data_preprocessing_24channel_multi(starting_points, selected_ligand_atom_pair, key_ligand):
    ...
    random_atoms_data = np.random.permutation(atoms_data)
    random_true_ligand_atoms = np.vstack([true_ligand, ligand_atoms_data, random_atoms_data])
    temp_true_dm = distance_matrix(random_true_ligand_atoms.T[1:].T, random_true_ligand_atoms.T[1:].T)
    ...
    for key_path in range(1):
        ...
        generated_locs = starting_points
        ...
        processed_data = np.concatenate((fake_dm_atoms_type, true_dm_atoms_type), axis=2)
        assert processed_data.shape == (24, 55, 110)

    return processed_data, generated_locs, random_atoms_data
...
if __name__ == '__main__':
    ...
    input_data_24channel = data_preprocessing_24channel('/','test')
