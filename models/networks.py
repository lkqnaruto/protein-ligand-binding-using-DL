...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    Parameters:
        input_nc (int) -- the number of channels in input images                                                            input_nc = 24
        output_nc (int) -- the number of channels in output images                                                          output_nc = 24
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        model = [nn.ReflectionPad2d(5),
                 nn.Conv2d(input_nc, ngf, kernel_size=11, padding=0, bias=use_bias),
                 norm_layer(ngf),
                 nn.ReLU(True)]
        ...
        for i in range(n_downsampling):  # add downsampling layers
            mult = 2 ** i
            model += [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1, bias=use_bias) if i ==0 \
                          else nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=4, stride=2, padding=1, bias=use_bias),
...
class UnetSkipConnectionBlock(nn.Module):
    ...
    def __init__(self, outer_nc, inner_nc, input_nc=None,
                 submodule=None, outermost=False, innermost=False, norm_layer=nn.BatchNorm2d, use_dropout=False, dims = None, use_attention = False):
...
class PixelDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, norm_layer=nn.InstanceNorm2d):
        ...
        self.net = [
            nn.utils.spectral_norm(nn.Conv2d(input_nc, ndf, kernel_size=1, stride=1, padding=0)),
            nn.LeakyReLU(0.2, True),
            axial_layer.axial55s_128_3layers(),
            nn.utils.spectral_norm(nn.Conv2d(ndf, ndf * 2, kernel_size=1, stride=1, padding=0, bias=use_bias)),
            norm_layer(ndf * 2),
            nn.LeakyReLU(0.2, True),
            axial_layer.axial55s_256_3layers(),
            nn.utils.spectral_norm(nn.Conv2d(ndf * 2, 1, kernel_size=1, stride=1, padding=0, bias=use_bias))]
